...
void cScenarioSimChar::Update(double time_elapsed)
{
	...
	double update_step = time_elapsed / mNumUpdateSteps;  // in this case 1/600 (second)
	int num_update_steps = (time_elapsed == 0) ? 1 : mNumUpdateSteps;
	// if time has elapsed, num_update_steps is 20 in this case
	for (int i = 0; i < num_update_steps; ++i)// one loop runs for 1/30 seconds
	{

        mTime += update_step;

        int mTimeZoom = (int)mTime * 30;
        if ((mTimeZoom % 15) == 0){
            mCurrStepNum++;
        }
...
int cScenarioSimChar::GetCurrStepNum() const
{
    return mCurrStepNum;
}
...
double cScenarioSimChar::GetTargetFootLength(double time, int currStepNum) const{
    return 0;
}

double cScenarioSimChar::GetTargetFootHeight(double time, int currStepNum) const{
    return 0;
}